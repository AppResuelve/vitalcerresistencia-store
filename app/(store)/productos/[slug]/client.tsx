...
export default function ProductDetailClient({ product }: { product: any }) {
      ...
      sku.attributeValues.forEach((av) => {
        ...
        if (!groups[aId])
          groups[aId] = {
            ...
            unitType: av.attribute.unitType || null,
            ids: [],
            valueMap: {},
          };
  ...
  const unitGroup = useMemo(
    () => attributeGroups.find((g) => g.unitType),
    [attributeGroups],
  );
  ...
  const derivedSku = useMemo(() => {
    ...
    return (
      skus.find((sku) =>
        selected.every((vId) =>
          sku.attributeValues.some((av) => av.id === vId),
        ),
      ) || skus[0]
    );
  }, [skus, selectedValues]);
  ...
  const discountPct =
    derivedSku?.discountPercentage ?? product?.discountPercentage;
  const comparePrice = derivedSku?.comparePrice ?? product?.comparePrice;
  const wholesalePrice = derivedSku?.wholesalePrice ?? product?.wholesalePrice;
  const wholesaleMinQty =
    derivedSku?.wholesaleMinQty ?? product?.wholesaleMinQty;
  ...
  useEffect(() => {
    ...
    attributeGroups.forEach((g) => {
      if (g.ids.length > 0) init[g.attributeId] = g.ids[0];
    });
          ...
          <div>
            ...
            {product.tagValues?.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-3">
                {product.tagValues.map((tv) => (
                  <Badge key={tv.id} color={tv.tag?.color}>
                    {tv.value}
                  </Badge>
                ))}
              </div>
                ...
                {attributeGroups.map((group) => (
                  <div key={group.attributeId}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {group.ids.map((vId) => {
                        const selected =
                          selectedValues[group.attributeId] === vId;
                        return (
                          <button
                            key={vId}
                            onClick={() =>
                          ...
                          >
                            {group.unitType
                              ? `${group.valueMap[vId]} ${UNIT_LABEL[group.unitType]}`
                              : group.valueMap[vId]}
                          </button>
                        );
                      })}
            ...
            <div className="mb-1">
              {unitGroup && (
                <p
                  ...
                  style={{ color: "var(--color-text-muted)" }}
                >
                  Precio por {UNIT_BASE_LABEL[unitGroup.unitType]}:{" "}
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {unitGroup
                    ? selectedValues[unitGroup.attributeId]
                      ? `${unitGroup.valueMap[selectedValues[unitGroup.attributeId]]} ${UNIT_LABEL[unitGroup.unitType]}`
                      : `x 1 ${UNIT_LABEL[unitGroup.unitType]}`
                    : "x 1 u."}
                </span>
              </div>
              {discountPct && comparePrice && (
                <span
            ...
            {hasWholesale && (
              ...
              >
                ...
                >
                  Precio mayorista · a partir de {wholesaleMinQty} unidades
                </p>
                <div className="flex items-center gap-2">
                  <span
                    style={{
